...
Instead of choosing among ~79 individual `kern_*` tools, call the single
...
without having to know which of the 83 tools fits.
...
Prefer `kern_meta` as your default. Set `KERN_MCP_SINGLE_TOOL=1` to expose
ONLY this tool to agents (useful when an agent is overwhelmed by the full
catalog). The classifier is deterministic keyword matching — no LLM, no
...
7. **Do I need to search the web/docs?** → call `kern_doc_fetch` then
   `kern_doc_search`, NOT `webfetch`/`websearch`.
...
### When to use full/raw (escape the compact default)

The compact routing truncates exact content by design. The default routes
(`read`→`kern_compact_file`, `grep`→`kern_ast_search`, `glob`→
`kern_project_map`, `bash` build/test→`kern_run_build`) return symbolic
summaries, symbol matches, or exit-status+errors only — NOT verbatim text,
NOT raw grep output, NOT full stdout. That is correct for discovery and
overview, but it drops exactly the detail a task sometimes needs (a specific
constant value, a function body, an exact line, full command output).

Switch to the explicit per-call bypass when the task needs verbatim detail:

| Need                              | Use                                           |
|-----------------------------------|-----------------------------------------------|
| Verbatim file text                | `read(path, full=true)`                       |
| Raw grep matches (not symbols)    | `grep(pattern, raw=true)`                     |
| Plain file listing (not symbol map)| `glob(pattern, raw=true)`                    |
| Full bash stdout                  | `nohup cmd > /tmp/out.txt 2>&1 &` then `cat`  |

**Criterion:** use the bypass when you need a specific value, function body,
exact line, regex pattern, or full command output. Use the compact default
for discovery, overview, "what's in this file", and "what calls X".

Known issue: in some repos the `bash` tool intermittently times out at
~30-300ms through the kern routing layer (a proxy-intercept issue, not the
command). `raw: true` does NOT reliably bypass it. Workarounds: retry the
same command 1-3 times, or use the `nohup ... > file; cat` pattern above.
...
`kern` ships ~79 `kern_*` MCP tools across these domains. If you are unsure
